...
int RMQ(int segment[], int qlow, int qhigh, int low, int high, int pos)
{
	...
	int mid = (low+high)/2;
	return min(RMQ(segment, qlow, qhigh, low, mid, 2*pos+1), RMQ(segment, qlow, qhigh, mid+1, high, 2+pos+2));
}
